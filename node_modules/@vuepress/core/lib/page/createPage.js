...
const createPage = async (app, options) => {
    ...
    const { renderedContent, deps, headers, hoistedTags, links, title, } = await renderPageContent_1.renderPageContent({
        app,
        content,
        filePath,
        filePathRelative,
    ...
    const { componentFilePath, componentFilePathRelative, componentFileContent, componentFileChunkName, } = await resolvePageComponentInfo_1.resolvePageComponentInfo({
        app,
        renderedContent,
        hoistedTags,
